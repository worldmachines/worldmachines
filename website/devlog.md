...
## 2026-07-15 · aneesh
Cutover moving the whole stack **back** to Aneesh's Cloudflare account for full control (Venkat re-points `worldmachines.org` DNS once verified). The backend never actually left — Oracle, Witness, and both DuckLake catalogs were still live on Aneesh's account — so this was mostly a website-layer rebuild plus a credential swap. Done: republished both catalogs current (835 notes + chunks); created the `worldmachines-notes`/`worldmachines-library` buckets, a fresh `HANDLES` KV, and the `worldmachines` Pages project; deployed the site to `worldmachines-2rd.pages.dev` (Oracle verified end-to-end through the site proxy); repointed `website/wrangler.jsonc` (`HANDLES` id + `ORACLE_URL`); swapped the repo's GitHub deploy secrets to Aneesh's account; and added a notes→catalog CI (`lake publish --only notes,edges`) in wm-encyclopedia-kb. See `DEPLOY-HANDOFF.md` for the runbook and Venkat's remaining action items (HANDLES export + DNS). Also merged the backlogged raw-notes PR #19.
