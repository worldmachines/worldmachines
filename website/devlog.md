...
## 2026-07-15 · aneesh
Started the cutover moving the whole stack **back** to Aneesh's Cloudflare account for full control (Venkat re-points `worldmachines.org` DNS once verified). The backend never actually left — Oracle, Witness, and both DuckLake catalogs are still live on Aneesh's account — so this is mostly a website-layer rebuild plus a credential swap: created the `worldmachines-notes`/`worldmachines-library` buckets, a fresh `HANDLES` KV, and the `worldmachines` Pages project on Aneesh's account, and repointed `website/wrangler.jsonc` (`HANDLES` id + `ORACLE_URL`). See `DEPLOY-HANDOFF.md` for the runbook and Venkat's action items (HANDLES export + DNS). Also merged the backlogged raw-notes PR #19.
