...
## 2026-05-30 · vgr
Added a full-text library system. New `worldmachines-library` R2 bucket stores PDFs under `public/` (open access, served to all) and `private/` (team-only, gated by CF Access JWT). The article JSON schema gained `license` and `pdf_key` fields; the resources page now shows PDF download badges for open-license documents and dynamically loads a "Team Library" section for authenticated members. First document: Darnton's _The Business of Enlightenment_ (CC BY). Also opened [PR #13](https://github.com/worldmachines/worldmachines/pull/13) proposing ADR 0002 — how to ingest library books into the Oracle (separate Parquet vs merged index), seeking input from Aneesh before building `tools/ingest_pdf.py`. Finally, wrote `tools/translate_french.py`: a Claude-powered script that extracts every French passage from a PDF, translates it, and outputs a supplement document with page numbers and chapter/section context — running now on the Darnton book to produce an English-translation companion for all contributors.
